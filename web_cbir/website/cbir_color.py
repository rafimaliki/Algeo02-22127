...
def get_result_images_color():
    ...
    similarities = [[os.path.basename(path), similarity_value] for path, similarity_value in zip(dataset_paths, [cosineSimAvg(v1, Result(path,4)) * 100 for path in dataset_paths]) if similarity_value > 60]
